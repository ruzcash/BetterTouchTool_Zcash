...
Touch Bar presets for live crypto prices, with a Zcash details submenu for
shielded supply and net shielded flow.
...
Binance, Coinbase, OKX, and KuCoin include six top-level coins:
`BTC`, `ETH`, `ZEC`, `LTC`, `FIL`, and `ZEN`.

Gemini and NEAR Intents stay on the shorter four-coin layout:
`BTC`, `ETH`, `ZEC`, and `LTC`.

All presets keep the same `Zcash Details` submenu. Provider variants
...
- Base coin order is still `BTC`, `ETH`, `ZEC`, `LTC`
- Binance, Coinbase, OKX, and KuCoin append `FIL`, then `ZEN`
...
- Formatting: BTC and ETH use whole dollars; ZEC, LTC, FIL, and ZEN use two
  decimals below `$1,000` and whole dollars at `$1,000` or above
- Theme: black buttons, white text, embedded coin icons
- BTC, ETH, LTC, FIL, ZEN, and the inner ZEC button open their exchange pages
...
- Every provider refresh retrieves all configured symbols in one public request.
